/*
  # Backfill token_character_alignments

  ## Overview
  Populates `token_character_alignments` with akshara-by-akshara
  alignments for every existing `verse_tokens` row. The segmentation
  logic mirrors the TypeScript runtime fallback in `AksharaSegmenter.ts`
  so curated and runtime breakdowns stay consistent.

  ## Tables Touched
    - token_character_alignments (INSERT only; idempotent via ON CONFLICT)
*/
...
ON CONFLICT (verse_token_id, position) DO NOTHING;
